document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("/api/auth/getUser", {
        ...
        .then(user => {
            ...
            loadReviews(user.username); // Start loading reviews
            loadBooks();
        })
        .catch(() => {
        ...
        fetch("api/auth/getReviews", {
            ...
            .then(data => {
                ...
                for (const review of userReviews) {
                    ...
                    reviewDiv.style.position = "relative"; // Make the card a positioning context
                    reviewDiv.innerHTML = `
...
`;
                    ...
                    reviewDiv.querySelector(".delete-review-btn").addEventListener("click", () => {
                        ...
                        if (confirm("Are you sure you want to delete this review?")) {
                            fetch(`/api/auth/deleteReview?bookId=${encodeURIComponent(bookId)}`, {
                                method: "GET",
                                headers: {
                                    "Authorization": token
                                }
                                ...
                                .then(res => res.json())
                                .then(() => {
                                    reviewDiv.remove(); // Remove from UI
                                })
                                .catch(err => {
                ...
                if (userReviews.length === pageSize) {
    ...
    <div style="position:absolute; top:8px; right:8px; display:flex; gap:4px;">
        <button class="btn btn-secondary edit-book-btn" data-id="${book.bookId}" title="Edit Status" style="padding:4px 8px;">
            <img src="images/edit.svg" alt="Edit" style="width:16px;height:16px;vertical-align:middle;">
        </button>
        <button class="btn btn-secondary delete-book-btn" data-id="${book.bookId}" title="Remove Book" style="padding:4px 8px;">
            <img src="images/trash.svg" alt="Delete" style="width:16px;height:16px;vertical-align:middle;">
        </button>
    </div>
    <h3>${book.title}</h3>
...
`;
                    ...
                    bookDiv.querySelector(".edit-book-btn").addEventListener("click", () => {
                        ...
                        if (newStatus && newStatus !== book.Status) {
                            fetch(`/api/auth/editBookStatus?bookId=${book.bookId}&Status=${newStatus}`, {
                                ...
                                .then(response => {
                                    console.log(response.message); // Optional: log server message
                                    bookDiv.querySelector(".book-status").textContent = newStatus; // Update UI
                                })
                                .catch(err => {
                    ...
                    bookDiv.querySelector(".delete-book-btn").addEventListener("click", () => {
                        if (confirm("Are you sure you want to remove this book from your shelf?")) {
                            fetch(`/api/auth/deleteBookFromShelf?bookId=${book.bookId}`, {
                                ...
                                .then(response => {
                                    console.log(response.message); // Optional: log server message
                                    bookDiv.remove(); // Success — update UI
                                })
                                .catch(err => {
                ...
                if ((booksPage + 1) * pageSize < data.length) {
